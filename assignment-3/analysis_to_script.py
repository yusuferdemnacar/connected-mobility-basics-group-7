...
output_file = "measurements/measurements.txt"
target_ip = "8.8.4.4"
...
fig.tight_layout()
fig.savefig("plots/number_of_measurements_per_probe_id.png")
plt.close(fig)
...
fig.savefig("plots/successful_vs_unsuccessful_ratio.png")
plt.close(fig)
...
_ = ax1.legend()
fig.savefig("plots/successful_vs_unsuccessful_by_probe_id.png")
plt.close(fig)
...
fig.savefig("plots/gateway_visibility.png")
plt.close(fig)
...
_ = fig.tight_layout()
fig.savefig("plots/gateway_visibility_by_success.png")
plt.close(fig)
...
fig.savefig("plots/successful_measurements_by_probe_id_and_gateway_presence.png")
plt.close(fig)
...
fig.tight_layout()
fig.savefig("plots/rtt_distribution_successful_gateway_present.png")
plt.close(fig)
...
fig.tight_layout()
fig.savefig("plots/rtt_time_evolution_per_probe_id.png")
plt.close(fig)
...
for continent, info in continents.items():
    ...
    if data:
        ...
        for label in ax1.get_xticklabels():
            ...
            label.set_ha('right')
        fig.tight_layout()
        fig.savefig(f"plots/rtt_distribution_{continent.lower().replace(' ', '_')}.png")
        plt.close(fig)
...
for continent, info in continents.items():
    ...
    if data:
        ...
        fig.tight_layout()
        fig.savefig(f"plots/rtt_time_evolution_{continent.lower().replace(' ', '_')}.png")
        plt.close(fig)
